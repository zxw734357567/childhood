...
/**
 * @author zxw
 * 需要使用jackson 来序列化java对象,这个必须得到满足,这个可以参照RedisTokenStore来定义
 * 以下三项就是必须定义序列化和反序列化的,因为你不能修改别人的jar包中的类,只能在代码中去定义序列化和反序列话的方式
 ...
 */
public class JojoRedisTokenStore  implements TokenStore {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        //从authentication 获取key值,
        // userName(如果存在的话)+clientId+scope，通过MD5加密得到的key,这个可以自定义的
    ...
    //  ACCESS : OAuth2AccessToken.getValue(),    OAuth2AccessToken
    //  AUTH : OAuth2AccessToken.getValue(),     OAuth2Authentication
    //  AUTH_TO_ACCESS : MD5(userName(存在的话)+clientId+Scope),     OAuth2AccessToken
    //  UNAME_TO_ACCESS :userName(存在的话)+clientId,     OAuth2AccessToken
    //  CLIENT_ID_TO_ACCESS :clientId,     OAuth2AccessToken
    //  REFRESH_TO_ACCESS :OAuth2AccessToken.getRefreshToken().getValue(),     OAuth2AccessToken.getValue()
    //  ACCESS_TO_REFRESH :OAuth2AccessToken.getValue(),     OAuth2AccessToken.getRefreshToken().getValue()
